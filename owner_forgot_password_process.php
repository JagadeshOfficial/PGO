...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($result->num_rows > 0) {
        ...
        $stmt = $conn->prepare("UPDATE owners SET reset_token = ? WHERE email = ?"); // Use 'owners' here
        $stmt->bind_param("ss", $token, $email);
        $stmt->execute();

        // Send the email using PHPMailer
        $resetLink = "http://localhost:8000/owner_reset_password.php?token=" . $token; // Adjust the port and URL as needed
